        ...
        for j in range(4):  ## 상하 좌우 비교
            ...
            if 0 <= cx < n and 0 <= cy < n:
                if cx == exitx and cy == exity:  ## 다음 방향에 탈출구 있을때,
                    ...
                    mvcnt += 1
                    continue  ## 밑에 다 패스
                if (maze[cx][cy] == 0) or (maze[cx][cy] == '*'):  ## 빈곳이거나, 거기 안에 참가자 있는 경우(깔 수 있는 곳)
...
def findSquare():
    ...
    for x, y in persons:
        ...
        rightx = max(x, exitx)
        leftx = rightx - maxdist

        righty = max(y, exity)
        lefty = righty - maxdist

        if leftx < 0:  ## 만약 범위를 벗어난다면,
            leftx = 0
            rightx = leftx + maxdist

        if lefty < 0:
            lefty = 0
            righty = lefty + maxdist
        ...
        for j in range(boxwidth):
            ...
            if type(boxary[i][j]) == str:
                ...
                if boxary[i][j] == '*':  ## 원래 참가자의 위치고, 이동할 방향의 값이 0이라면 이동 가능
                    # print(i,j)
                    # personcnt = persons.count((i + mx, j + my))

                    # if personcnt > 1 : ##참가자가 여러명이라면?
                    reqcnt = personcnt[(i + mx, j + my)]  ## 원래 계산되어있던 카운트 수 만큼
